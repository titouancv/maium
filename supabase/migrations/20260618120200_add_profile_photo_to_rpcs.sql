-- Surface each user's custom profile photo through the summary-returning RPCs so
-- avatars in the suggestions list and the stories feed (author, original author
-- and viewers) use it instead of the default. Recreated from their latest
-- definitions (20260617120000 / 20260614120000), with profile_photo added.

-- ── Suggested users ─────────────────────────────────────────────────────────
-- Adding profile_photo changes the return type, which CREATE OR REPLACE cannot
-- do (Postgres 42P13) — drop first, then recreate.
