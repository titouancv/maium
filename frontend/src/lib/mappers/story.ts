...
export type DbStoryRaw = {
  ...
  created_at: string;
  original_author?: { pseudo: string | null } | null;
};
...
export const STORY_SELECT = `
  ...
  original_author:original_author_id(pseudo)
`.trim();
...
export function mapStoryFromDb(
  raw: DbStoryRaw,
  flags: { seen: boolean; likedByMe: boolean },
): StoryData {
  return {
    id: raw.id,
    ...
    originalAuthorPseudo: raw.original_author?.pseudo ?? null,
